...
public class CategoriesController {
    ...
    public ResponseEntity<?> saveOrUpdate(@RequestBody Category category) {
        try {
            Category existed = categoriesService.getByName(category.getCategoryName());
            Integer catId = category.getId();
            Integer existedId = existed != null ? existed.getId() : null;
            if (existed != null && (catId == null || !existedId.equals(catId))) {
